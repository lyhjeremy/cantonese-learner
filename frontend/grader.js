...
// Single-shot recognition for iOS: no `continuous`, no `interimResults`, no
// restart loop. iOS ends the session on its own when you stop speaking; we then
// resolve with whatever it heard. A hard cap guarantees resolution.
function listenSingleShot(rec, { onStart, maxMs }) {
...
export function listen({ lang = "yue-Hant-HK", onStart, silenceMs = 3500, preSpeechMs = 8000, maxMs = 45000 } = {}) {
  ...
  // iPhone/iPad: use single-shot recognition (continuous mode aborts on iOS).
  if (isIOSLike()) return listenSingleShot(rec, { onStart, maxMs });
